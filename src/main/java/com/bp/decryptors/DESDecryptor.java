...
public class DESDecryptor implements Decryptor {
    ...
    public JPanel createOperationPanel(ResultCallback callback) {
        ...
        settingsPanel.setBorder(new TitledBorder("DES配置"));
        ...
        String[] modes = {"DES/ECB/PKCS5Padding", "DES/CBC/PKCS5Padding", "DES/CFB/PKCS5Padding", "DES/OFB/PKCS5Padding"};
        ...
        decryptButton.addActionListener(e -> {
            try {
                ...
                if (keyBytes.length < 8) {
                    keyBytesAdjusted = new byte[8];
                    System.arraycopy(keyBytes, 0, keyBytesAdjusted, 0, keyBytes.length);
                } else if (keyBytes.length > 8) {
                    keyBytesAdjusted = new byte[8];
                    System.arraycopy(keyBytes, 0, keyBytesAdjusted, 0, 8);
                } else {
                    keyBytesAdjusted = keyBytes;
                }
                
                SecretKeySpec secretKey = new SecretKeySpec(keyBytesAdjusted, "DES");
                ...
                JOptionPane.showMessageDialog(panel, "DES解密失败: " + ex.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
                System.err.println("DES解密错误: " + ex.getMessage());
            }
        });
        ...
        encryptButton.addActionListener(e -> {
            try {
                ...
                JOptionPane.showMessageDialog(panel, "DES加密失败: " + ex.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
                System.err.println("DES加密错误: " + ex.getMessage());
            }
        });
